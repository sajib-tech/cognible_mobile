...
class ClinicTaskList extends Component {
    ...
    getData() {
        ...
        TherapistRequest.getTaskList().then(dashboardResult => {
            ...
            tasks = _.orderBy(tasks, ['node.createdAt'], ['desc']);

            this.setState({
                isLoading: false,
                tasks, unfilteredTasks: tasks
            });
        }).catch(error => {
            console.log(error);
                ...
                {this.state.tasks?.map((task, index) => {
                    ...
                    return (
                        <TouchableOpacity key={index} onPress={() => {
                            ...
                            {task?.node?.status?.taskStatus == "Open" && <TouchableOpacity style={{ padding: 5 }} onPress={() => {
                                Alert.alert(
                                    'Information',
                                ...
                                <MaterialCommunityIcons name='check-circle-outline' size={30} color={Color.success} />
                            </TouchableOpacity>}
...
export default connect(mapStateToProps, mapDispatchToProps)(ClinicTaskList);
