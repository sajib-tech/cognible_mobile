...
// class ScreeningResultWaiting extends Component {
//   constructor(props) {
//     super(props);
//     this.state = {
//       isLoading: false,
...
//                 },
//               ]}>
//               {getStr('NewUpdated.DesPedi').replace(
//                 '{studentName}',
...
//         <NavigationHeader
//           title={getStr('homeScreenAutism.ScreeningResults')}
...
class ScreeningResultWaiting extends Component {
  ...
  getScreeningresult = () => {
    const variables = {
      pk: this.props?.route?.params?.screeningId,
      // test: false,
    };
    this.setState({isLoading: true})

    ParentRequest.getScreeningResult(variables).then((res) => {

      this.setState({cardDesc: res.data.preliminaryAssessmentFinalResponse.result})
      if (res.data.preliminaryAssessmentFinalResponse.result.includes('NOT'))  {
        this.setState({isAtRisk: "green", isLoading: false})
      } else {
        this.setState({isAtRisk: 'red', isLoading: false})
      }
  ...
  submit = () => {
    const {feedback, sliderValue} = this.state;

    const variables = {
      id: store.getState().user.userType.id,
      rating: sliderValue,
      feedback: feedback,
    };

    ParentRequest.submitResult(variables)
      .then((res) => {
        Alert.alert(
          'Session Feedback',
          ...
          [
            {
              ...
              onPress: () => {
                ...
                this.props.navigation.navigate('ScreeningAnalyzingAssess', {
  ...
  */
  ...
  renderList() {
    ...
    let studentName
    if(this.props.route.params && this.props.route.params.student) {
    ...
    return (
      <>
        <ScrollView
          ...
          {scoreInt == 0 && (
            <TouchableOpacity
              style={[styles.header, {backgroundColor: this.state.isAtRisk} ]}
              activeOpacity={0.8}
              onPress={() => {
                //this.setState({ isScoreShow: true });
              }}>
              {this.state.cardDesc === null ? (
              <><Text style={Styles.veryBigWhiteTextBold}>
                {getStr('NewUpdated.WaitingforResults')}
              </Text>
              <Text style={[Styles.smallWhiteText, {marginVertical: 8}]}>
                {getStr('NewUpdated.waitDes')}
            </Text> </>) : (<Text style={[Styles.smallWhiteText, {marginVertical: 8}]}>{this.state.cardDesc}</Text>)}
              <View style={[styles.line, {marginVertical: 8, opacity: 0.4}]} />
              <View style={[Styles.row, {alignItems: 'center'}]}>
                <Text style={[Styles.whiteText, {marginVertical: 8}]}>
                  {getStr('NewUpdated.ContactYourPedistrician')}
          ...
          <View
            ...
            <Text
              ...
              {this.props.route.params.student ? getStr('homeScreenAutism.ScreeningDesc') +
               this.props.route.params.student.node.firstname +
                getStr('homeScreenAutism.ScreenDesc') :getStr('homeScreenAutism.ScreeningDesc') +StudentHelper.getStudentName() + getStr('homeScreenAutism.ScreenDesc') }
              {/* {getStr('NewUpdated.DesPedi').replace(
                '{studentName}',
                StudentHelper.getStudentName(),
              )} */}
