...
class ParentTaskNew extends Component {
  constructor(props) {
    super(props);
        ...
        this.setState(
          ...
          () => {
            if (this.params) {
              this.setState({
                selectedStudent:
                  this.params.node.students.edges.length != 0
                    ? selectedStudents
                    : [],
                selectedPriority:
                  this.params?.node?.priority?.id != null
                    ? this.params?.node?.priority?.id
                    : '',
                selectedStaff:
                  this.params.node.assignWork.edges.length != 0
                    ? selectedStaffs
                    : [],
                selectedTaskType: this.params?.node?.taskType.id,
                selectedStatus:
                  this.params?.node?.status?.id != null
                    ? this.params.node.status.id
                    : '',
                taskName: this?.params?.node?.taskName,
                description: this.params.node.description,
                reminders:
                  this.params.node.remainders.edges.length != 0
                    ? reminders
                    : [
                        {
                          time: moment().format('HH:mm:ss'),
  ...
  saveTask() {
    ...
    let queryString = {
      ...
      Date,
      time,
      taskDatetime: moment(Date + ' ' + time, 'YYYY-MM-DDTHH:mm:ssZ')
        .local()
        .utc()
        .format('YYYY-MM-DDTHH:mm:ssZ'),
      assignWork: selectedStaff,
      students: [StudentHelper.getStudentId()],
    };
    ...
    return (
      <SafeAreaView style={styles.container}>
        <NavigationHeader
          ...
          title={this.params ? getStr("TargetAllocate.EditTask") : getStr("TargetAllocate.NewTask")}
          // disabledTitle
        />
        ...
        {!this.state.isLoading && (
          <Container enablePadding={this.props.disableNavigation != true}>
            <ScrollView
              ...
              <PickerModal
                label={getStr("TargetAllocate.Type")}
              ...
              {(this.state.typeIndex == 0 ||
                ...
                <TextInput
                  label={getStr("TargetAllocate.TaskName")}
              ...
              {(this.state.typeIndex == 0 ||
                ...
                this.state.typeIndex == 3) && (
                <TextInput
                  label={getStr("TargetAllocate.TaskSummary")}
              ...
              {(this.state.typeIndex == 0 ||
                ...
                this.state.typeIndex == 2) && (
                <View>
                  <Text style={Styles.grayText}>{getStr("ExtraAdd.StartDate")}</Text>
                  <DateInput
                    format="YYYY-MM-DD"
              ...
              {(this.state.typeIndex == 0 ||
                ...
                this.state.typeIndex == 2) && (
                <View>
                  <Text style={Styles.grayText}>{getStr("ExtraAdd.EndDate")}</Text>
                  <DateInput
                    format="YYYY-MM-DD"
                    displayFormat="dddd, DD MMM YYYY"
                    value={dueDate}
              ...
              {(this.state.typeIndex == 0 ||
                ...
                this.state.typeIndex == 2) && (
                <View>
                  <Text style={Styles.grayText}>{getStr("TargetAllocate.Reminder")}</Text>
                    ...
                    <Row>
                      ...
                      <Column style={{flex: 0, width: 120}}>
                        <Button
                          theme="secondary"
                          style={{height: 42}}
                          labelButton={getStr("Profile.Add")}
                          onPress={() => {
                            let reminders = this.state.reminders;
                            reminders.push({
                              time: moment().format('HH:mm:ss'),
                        ...
                        <Text
                          style={{
                            ...
                            flex: 1,
														...
														{getStr("TargetAllocate.TaskCompletion")}
            ...
            <Button
              labelButton={getStr("TargetAllocate.SaveTask")}
...
export default connect(mapStateToProps, mapDispatchToProps)(ParentTaskNew);
