...
class SessionBehaviorScreen extends Component {
  constructor(props) {
    super(props);
  ...
  renderList() {
    let {decelItems, environments,weeks,searchText,tempid} = this.state;
    const filtered=decelItems.filter(createFilter(searchText, KEYS_TO_FILTERS))
    return (
      <>
        <View style={{height: 10}} />

        {filtered.length === 0 && <NoData>No Data Available</NoData>}
        ...
        {filtered.map(({node}, index) => {
            ...
            return (
              <BehaviourDecelCard
                ...
                tempId={node.id}
                isFromSession={true}
                sessionNavigation={() => this.props.sessionNavigation()}
                sessionId={this.props.route.params.childSessionId}
  ...
  editTemplate(template) {
    ...
    console.log("id==>",template);
      ...
      if(this.props.isFromSession) this.props.sessionNavigation()		
