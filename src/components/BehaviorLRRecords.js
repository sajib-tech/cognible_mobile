...
const BehaviorLRRecords = (props) => {
      ...
      .then((result) => {
        ...
        result.data.getBehaviorTemplates.edges.map((element, index) => {
          if (element.node.id === tempId) {
            ...
            if (behaviorType.includes('LT')) {
              if (latencybehaviorrecordSet) {
                const behaviorRecords = latencybehaviorrecordSet.edges.map(
                  ({node}) => ({
                    id: node.id,
              date: node.date,
  ...
  return (
    <SafeAreaView style={{backgroundColor: '#fff', flex: 1}}>
      <NavigationHeader
        title={`${getStr('BegaviourData.BehaviorLR')}-${behaviorName}`}
                  ...
                  <TouchableOpacity
                   style={{marginRight: 8}}
                    onPress={() => {
                      ...
                      props.navigation.navigate('BehaviorLRScreen', {
...
export default connect(mapStateToProps, mapDispatchToProps)(BehaviorLRRecords);
