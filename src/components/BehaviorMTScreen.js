...
const BehaviorMTScreen = (props) => {
  ...
  const [considerStartInterval, setConsiderStartInterval] = useState(null);
  ...
  useEffect(()=>{
    if(props.route.params.currentRecord){
      ...
      setRangeValue([data.rangeMin,data.rangeMax])
      const checkboxes = []
      const hited=[]
  ...
  const renderRailSelected = useCallback(
    () => <View style={{backgroundColor: 'blue', height: 5, flex: 1}} />,
    [],
  );
  const renderLabel = useCallback((value) => <Text>{value}</Text>, []);
  const renderNotch = useCallback(() => <Text>|</Text>, []);
  ...
  useEffect(() => {
    ...
    console.log(currentTime, currentInterval);
    const intervalTime = selectedLengthOfInterval * 60;
    ...
    if (behaviorStarted) {
      ...
      if (behaviorStartTime != null) {
        ...
        // let startInterval = Math.floor(behaviorStartTime / intervalTime) + 1;
        let considerStart = false;
        let considerEnd = false;
        let startInterval =
          parseInt(
            (behaviorStartTime - 1) / (selectedLengthOfInterval * 60),
            10,
          ) + 1;
        let endInterval =
          parseInt((currentTime - 1) / (selectedLengthOfInterval * 60), 10) + 1;

        let actualStartInterval = startInterval;
        let actualEndInterval = endInterval;
        if ((behaviorStartTime * 100) / intervalTime <= rangeValue[0]) {
          considerStart = true;
        }

        if ((currentTime * 100) / intervalTime >= rangeValue[1]) {
          considerEnd = true;
        }

        console.log(startInterval, 'ConsiderStart', considerStart);
        console.log(endInterval, 'ConsiderEnd', considerEnd);

        if (startInterval == endInterval) {
          if (considerStart && considerEnd) {
            setHitCheckbox([startInterval]);
          }
        } else {
          if (!considerStart) {
            actualStartInterval += 1;
          }
          if (!considerEnd) {
            actualEndInterval -= 1;
          }
          ...
          const x = [];
          for (let i = actualStartInterval; i <= actualEndInterval; i++) {
            x.push(i);
          }
          setHitCheckbox(x);
        }
        ...
        // if((behaviorStartTime*100)/intervalTime<=rangeValue[0]){
        //   considerStart = true;
        //   startInterval = parseInt(
        //     (behaviorStartTime - 1) / (selectedLengthOfInterval * 60),
        //     10,
        //   );
        // }else if (
        //   (behaviorStartTime * 100) / intervalTime >= rangeValue[0] &&
        //   (behaviorStartTime * 100) / intervalTime <= rangeValue[1]
        // ){
        //   considerStart = true
        //   startInterval = parseInt(
        //     (behaviorStartTime - 1) / (selectedLengthOfInterval * 60),
        ...
        // if (
        //   (currentTime * 100) / intervalTime >= rangeValue[0] &&
        //   (currentTime * 100) / intervalTime <= rangeValue[1]
        // ){
        //   endInterval = parseInt(
        //     (currentTime - 1) / (selectedLengthOfInterval * 60),
        //     10,
        //   );
        // }
        ...
        // let endInterval = Math.floor(currentTime / intervalTime);
  ...
  const saveRecord = () => {
    ...
    if(props.route.params.currentRecord){
      ...
      setIsSaving(true);
      ParentRequest.updateMTBehaviorRecord(variables)
        .then((res) => {
          console.log('response--->', res);
          setIsSaving(false);
          Alert.alert(
            getStr('BegaviourData.BehaviorMT'),
            'Successfully Saved',
            [
              {
                text: 'OK',
      ...
      console.log('variables------->', variables);
      setIsSaving(true);
      ParentRequest.addMTBehaviorRecord(variables)
        .then((res) => {
          console.log('response--->', res);
          setIsSaving(false);
          Alert.alert(
  ...
  const handleDiscrete = () => {
    ...
    const intervalTime = selectedLengthOfInterval * 60;
    const localTime =
      currentTime - currentInterval * selectedLengthOfInterval * 60;
    console.log(localTime);
    if (
      (localTime * 100) / intervalTime >= rangeValue[0] &&
      (localTime * 100) / intervalTime <= rangeValue[1]
    ) {
      const temp = continuousArray;
      temp.push({startTime: currentTime, isSuccessful: true});
      setHitCheckbox([currentInterval + 1]);
      setContinuousArray(temp);
    } else {
      const temp = continuousArray;
  ...
  return (
    <SafeAreaView style={{backgroundColor: '#fff', flex: 1}}>
      <NavigationHeader
        title={getStr('BegaviourData.BehaviorMT')}
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={{flex: 1, alignItems: 'center'}}>
          ...
          <View
            ...
            <RangeSlider
              ...
              onValueChanged={(low, high, fromUser) => {
                ...
                if (rangeValue[0] != low || rangeValue[1] != high) {
          ...
          <View
            ...
            <Text style={{flex: 1}}>{rangeValue[0]}</Text>
...
export default connect(mapStateToProps, mapDispatchToProps)(BehaviorMTScreen);
