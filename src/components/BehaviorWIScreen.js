...
const BehaviorWIScreen = (props) => {
  ...
  const saveRecord = () => {
    ...
    if(props.route.params.currentRecord){
      ...
      setIsSaving(true);
      ParentRequest.updateWIBehaviorRecord(variables)
        .then((res) => {
          console.log('response--->', res);
          setIsSaving(false);
          Alert.alert(
            getStr('BegaviourData.BehaviorWI'),
            'Successfully Saved',
            [
              {
                text: 'OK',
      ...
      console.log('variables------->', variables);
      setIsSaving(true);
      ParentRequest.addWIBehaviorRecord(variables)
        .then((res) => {
          console.log('response--->', res);
          setIsSaving(false);
          Alert.alert(
  ...
  const afterTimerStarts = () => {
    ...
    setTimeInterval(
      setInterval(() => {
        ...
        // if (Math.floor(currentInterval + 1 / chunkSize) + 1) {
        //   setCurrentTable(currentInterval + 1);
        // }
  ...
  return (
    <SafeAreaView style={{backgroundColor: '#fff', flex: 1}}>
      <NavigationHeader
        title={getStr('BegaviourData.BehaviorWI')}
...
export default connect(mapStateToProps, mapDispatchToProps)(BehaviorWIScreen);
