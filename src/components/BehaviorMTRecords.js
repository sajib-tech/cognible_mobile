...
const BehaviorMTRecords = (props) => {
      ...
      .then((result) => {
        ...
        result.data.getBehaviorTemplates.edges.map((element, index) => {
          if (element.node.id === tempId) {
            ...
            if (behaviorType.includes('MT')) {
              if (momentarytimebehaviorrecordSet) {
                const behaviorRecords = momentarytimebehaviorrecordSet.edges.map(({ node }) => {
                    let count = 0
  ...
  return (
    <SafeAreaView style={{backgroundColor: '#fff', flex: 1}}>
      <NavigationHeader
        title={`${getStr('BegaviourData.BehaviorMT')}-${behaviorName}`}
                  ...
                  <TouchableOpacity
                   style={{marginRight: 8}}
                    onPress={() => {
                      ...
                      props.navigation.navigate('BehaviorMTScreen', {
...
export default connect(mapStateToProps, mapDispatchToProps)(BehaviorMTRecords);
