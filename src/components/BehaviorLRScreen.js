...
const BehaviorLRScreen = (props) => {
  ...
  useEffect(()=>{
    if(props.route.params.currentRecord){
      ...
      let sec = pad(data.latency % 60);
        let min = pad(parseInt(data.latency / 60));
        setShowTimer(min + ':' + sec);
        setTotalSeconds(data.latency);
      setDuration(data.latency)      
      setDate(data.date)
  ...
  const saveRecord = () => {
    ...
    ParentRequest.updateLatencyBehaviorRecord(variables)
      .then((res) => {
        console.log('response--->', res);
        setIsSaving(false);
        Alert.alert(
          getStr('BegaviourData.BehaviorLR'),
    ...
    console.log('variables------->', variables);
    ParentRequest.addLatencyBehaviorRecord(variables)
      .then((res) => {
        console.log('response--->', res);
        setIsSaving(false);
        Alert.alert(
  ...
  return (
    <SafeAreaView style={{backgroundColor: '#fff', flex: 1}}>
      <NavigationHeader
        title={getStr('BegaviourData.BehaviorLR')}
...
export default connect(mapStateToProps, mapDispatchToProps)(BehaviorLRScreen);
