...
const BehaviorPIRecords = (props) => {
      ...
      .then((result) => {
        ...
        result.data.getBehaviorTemplates.edges.map((element, index) => {
          if (element.node.id === tempId) {
            ...
            if (behaviorType.includes('PI')) {
              if (partialintervalbehaviorrecordSet) {
                const behaviorRecords = partialintervalbehaviorrecordSet.edges.map(({ node }) => {
                    let count = 0
  ...
  return (
    <SafeAreaView style={{backgroundColor: '#fff', flex: 1}}>
      <NavigationHeader
        title={`${getStr('BegaviourData.BehaviorPI')}-${behaviorName}`}
                  ...
                  <TouchableOpacity
                   style={{marginRight: 8}}
                    onPress={() => {
                      ...
                      props.navigation.navigate('BehaviorPIScreen', {
...
export default connect(mapStateToProps, mapDispatchToProps)(BehaviorPIRecords);
